...
# Clé Gemini / GCP : définir GEMINI_API_KEY dans .env (jamais en dur dans le code)
...
_api_key = _config("GEMINI_API_KEY", default="")
if not _api_key:
    raise RuntimeError(
...
# 2. Ta documentation UHAKIKAAPP (condensée pour le script)
DOCUMENTATION_UHAKIKAAPP = """
# UHAKIKAAPP

**Certitude. Fiabilité. Assurance.**
...
- Présentation générale de UHAKIKAAPP
- À qui s'adresse l'application ?
- Problèmes que l'application résout
- Fonctionnalités principales
- Lecture de code-barres (module prévu)
- Système d'abonnement (module prévu)
- Présentation de l'API
- Avantages de UHAKIKAAPP
...
- L'utilisateur s'inscrit (POST /api/users/).
- Il se connecte (POST /api/auth/).
- Il crée son entreprise (POST /api/entreprises/) — un membership admin est créé automatiquement.
- Il peut activer les succursales et créer des points de vente (POST /api/succursales/).
...
- Création du lot (LigneEntree avec quantite_restante = quantite).
- Augmentation de Stock.Qte.
- Mise à jour du seuilAlert de l'article.
- Sortie de caisse (MouvementCaisse SORTIE) si le paiement est immédiat — avec vérification du solde disponible.
...
- Consommation FIFO des lots (LigneSortieLot).
- Calcul des bénéfices par lot (BeneficeLot).
- Diminution de Stock.Qte.
- Entrée caisse (MouvementCaisse ENTREE) — montant 0 si vente à crédit.
...
- Le client ou l'admin crée une commande avec des lignes (article catalogue ou nom libre).
- Le staff valide ou rejette la commande.
- Au passage à LIVREE (première fois) :
...
UHAKIKAAPP expose tous les rapports métier en **JSON structuré** (le frontend gère l'affichage et l'export PDF).
#### Tableau des rapports disponibles
| Rapport | Endpoint JSON | Rôle et utilité |
| --- | --- | --- |
| Inventaire | GET /api/rapports/inventaire/ | État complet du stock : quantités, seuils, statuts. |
| Bon d'entrée (réquisition) | GET /api/rapports/bon-entree/ | Articles en rupture ou alerte à commander. |
| Bon d'achat | GET /api/rapports/bon-achat/ | Approvisionnements par période ou entrée. |
| Dettes client (détail) | GET /api/rapports/clients-dettes/?client_id=... | Dettes détaillées d'un client. |
| Dettes générales | GET /api/rapports/clients-dettes-general/ | Synthèse clients avec solde > 0. |
| Rapport des ventes | GET /api/rapports/ventes/ | Lignes de vente, FIFO, bénéfices. |
| Fiche stock | GET /api/rapports/{id}/fiche-stock/ | Mouvements FIFO d'un article. |
| Journal complet | GET /api/rapports/journal/ | Entrées, ventes, caisse, paiements dettes. |
...
| État caisse | GET /api/mouvements-caisse/solde/ | Soldes par devise |
...
- Chaque article pourra être associé à un code-barres unique (EAN-13, Code 128, QR…).
- Lors du scan, le système retrouvera automatiquement l'article correspondant.
- Intégration dans : création d'articles, recherche, vente à la caisse, inventaire.
...
- Création de l'entreprise et du catalogue (types : Homme, Femme, Enfant).
- Approvisionnements enregistrés avec prix d'achat et de vente.
- Ventes quotidiennes avec factures PDF.
- Ventes à crédit pour les clients fidèles → suivi des dettes et paiements partiels.
- Rapport mensuel des ventes et produits les plus vendus.
...
- Articles classés par type (Viandes, Légumes, Boissons).
- Entrées de stock avec dates d'expiration.
- Sorties journalières liées aux ventes.
- Alertes sur les produits proches de l'expiration (30 jours).
- Rapport mensuel des approvisionnements et des ventes.
...
- Catalogue complet avec codes articles automatiques.
- Ventes rapides via recherche full-text.
- Impression de tickets POS sur imprimante thermique 58 mm.
- (Futur) Scan code-barres pour identification instantanée des produits.
- Tableau de bord caisse en fin de journée.
...
- Entreprise avec has_branches activé et 2 succursales.
- Rôles : gérant (admin), agents (user) limités à leur succursale.
- Lots fournisseurs en transit → clôture → stock mis à jour.
- Ventes à crédit avec recouvrement et rapports dettes.
- Bénéfices totaux avec évaluation de performance mensuelle.
- Portail client pour les commandes en ligne.
...
8. GET  /api/rapports/inventaire/  → Rapport JSON
...
UHAKIKAAPP — Parce que votre entreprise mérite la certitude.
**Documentation officielle — Backend API**
...
1. Si la question porte sur UHAKIKAAPP, ses modules, ses fonctionnalités ou ses processus, utilise la documentation pour répondre de manière claire, concise et professionnelle.
2. Si la question demande des données en temps réel (ex: "combien d'articles reste-t-il en stock ?", "le solde de la caisse"), réponds EXACTEMENT ceci : "Je connais le fonctionnement théorique du module de stock/caisse, mais je ne suis pas encore connecté en temps réel à la base de données de votre application pour vous donner ces chiffres."
3. Si la question est hors-sujet (ex: recettes, code général, culture générale, poésie), tu dois obligatoirement refuser en répondant EXACTEMENT ceci : "Je suis un assistant dédié exclusivement à l'application UHAKIKAAPP. Je ne peux pas répondre à cette question."
"""

print("=== CHATBOT UHAKIKAAPP INITIALISÉ ===")
print("Posez vos questions (ou tapez 'quitter' pour arrêter)\n")

# 4. Boucle de discussion (Chat)
while True:
    ...
    try:
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                ...
                temperature=0.1 # Basse température pour éviter que l'IA n'invente des choses (hallucinations)
            )
        )
        print(f"UHAKIKA-BOT : {response.text}\n")
